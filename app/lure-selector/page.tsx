...
export default function LureSelectorPage(){
  ...
  useEffect(()=>{
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${spot.lat}&longitude=${spot.lon}&current=wind_speed_10m,cloud_cover,precipitation&daily=sunrise,sunset&timezone=auto&forecast_days=1`)
  ...
  const result=useMemo(()=>{
    ...
    const alternative=["Sardine / Silver","Blue / Silver"].includes(color)?"Gold / Pink":["Chartreuse / Orange","White / Orange Tiger"].includes(color)?"Silver / Red Head":"Sardine / Silver";
