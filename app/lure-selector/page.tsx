...
const resultArabic:Record<string,string> = {
  ...
  "Sardine / Silver":"سردين / فضي","Blue / Silver":"أزرق / فضي","Gold / Pink":"ذهبي / وردي","Chartreuse / Orange":"شارتروز / برتقالي","Dark Blue / Black":"أزرق داكن / أسود","Zebra Glow / Pink Glow":"زيبرا مضيء / وردي مضيء",
...
export default function LureSelectorPage(){
  ...
  const result=useMemo(()=>{
    ...
    else if(clarity==="murky"){color=lowLight?"Zebra Glow / Pink Glow":"Chartreuse / Orange";finish=lowLight?"Glow":"High contrast / glow accent";}
    else if(clarity==="stained"){color=lowLight?"Gold / Pink":"Chartreuse / Orange";finish=lowLight?"Warm flash":"Contrast flash";}
    else if(lowLight){color="Gold / Pink";finish="Warm flash";}
    ...
    const alternative=["Sardine / Silver","Blue / Silver"].includes(color)?"Gold / Pink":"Sardine / Silver";
  ...
  const lureShape=result.type==="Metal Jig"?"jig":["Popper","Topwater Pencil"].includes(result.type)?"popper":"minnow";
  const lureTone=result.color==="Sardine / Silver"?"natural":result.color==="Blue / Silver"?"blue-silver":result.color==="Chartreuse / Orange"?"chartreuse-orange":result.color==="Zebra Glow / Pink Glow"?"glow":result.color==="Dark Blue / Black"?"dark":"gold-pink";
