...
function Surfboard({ size = 16, ...props }: React.SVGProps<SVGSVGElement> & { size?: number }) {
  return (
...
function ForecastChart({
  title, unit, values, secondary, secondaryLabel, headingDetail, selectedHour, onSelect, accent = "cyan", icon: ChartIcon = Waves, scoreIcon: ScoreIcon = Fish, direction, directionFrom = false, currentDirection, currentDirectionFrom = false, currentSpeed, tideDetails, nowHour, nowTime, rtl = false, decimals = 1, xLabels, selectedLabel, rangeLabel, className = "", children,
}: {
  ...
  icon?: React.ElementType; scoreIcon?: React.ElementType; direction?: number; directionFrom?: boolean; currentDirection?: number; currentDirectionFrom?: boolean; currentSpeed?: number;
  ...
  const fishSchool = fishCount > 0 && <span className="fish-school" aria-label={rtl ? `${fishCount} رموز تمثل مستوى النشاط` : `${fishCount} icons representing the activity level`}>{Array.from({ length: fishCount }, (_, index) => <ScoreIcon aria-hidden="true" key={index} size={16}/>)}</span>;
    ...
    <main dir={rtl ? "rtl" : "ltr"} className={`site-shell ${rtl ? "font-arabic" : ""}`}>
      ...
      <section id="conditions" className="dashboard-wrap">
        ...
        <div className="forecast-explorer" id="plan-your-day">
          ...
          {activity === "surfing" && <>
            <ForecastChart className="chart-span-full" title={rtl ? "تقييم السيرف اليومي" : "Daily Surfing Activity"} icon={Waves} scoreIcon={Surfboard} unit="/100" values={surfingActivityByHour} selectedHour={selectedHour} onSelect={setSelectedHour} accent="cyan" decimals={0} nowHour={liveNowHour} nowTime={currentTime} rtl={rtl}><div className="chart-facts compact"><span>{rtl ? "النشاط الآن" : "Current conditions"}<strong>{(surfingActivityByHour[selectedHour] ?? 0) >= 72 ? t.good : (surfingActivityByHour[selectedHour] ?? 0) >= 48 ? t.caution : t.difficult}</strong></span><span>{rtl ? "أفضل سيرف" : "Best time"}<strong>{formatTime12(conditions.hourly[bestHour.index]?.time)}</strong></span></div></ForecastChart>
          ...
          {activity === "kayaking" && <>
            <ForecastChart className="chart-span-full" title={rtl ? "تقييم الكاياك اليومي" : "Daily Kayaking Activity"} icon={ShipWheel} scoreIcon={Kayak} unit="/100" values={kayakingActivityByHour} selectedHour={selectedHour} onSelect={setSelectedHour} accent="cyan" decimals={0} nowHour={liveNowHour} nowTime={currentTime} rtl={rtl}><div className="chart-facts compact"><span>{rtl ? "النشاط الآن" : "Current conditions"}<strong>{(kayakingActivityByHour[selectedHour] ?? 0) >= 72 ? t.good : (kayakingActivityByHour[selectedHour] ?? 0) >= 48 ? t.caution : t.difficult}</strong></span><span>{rtl ? "أفضل وقت آمن" : "Best time"}<strong>{formatTime12(conditions.hourly[bestHour.index]?.time)}</strong></span></div></ForecastChart>
