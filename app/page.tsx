...
type Metric = { label:string; value:string; sub:string; icon:React.ElementType; emphasis?:"wind"|"wave" };
type Conditions = {
...
function ForecastChart({
  ...
  return (
    <article className={`forecast-chart chart-${accent} ${tideDetails ? "tide-chart" : ""}`}>
      <div className="chart-heading">
        <div className="chart-primary-heading"><div className="chart-title-line"><span><ChartIcon size={19}/></span><p>{title}</p></div><strong>{value.toFixed(decimals)} <small>{unit}</small></strong></div>
...
export default function Home() {
  ...
  const selectWeekDay = (index: number) => {
    ...
    setSelectedHour(index === 0 ? Math.floor(currentDecimalHour) : weeklyData[index]?.bestHour ?? 9);
  };
  ...
  const windMetric: Metric = { label: rtl ? "الرياح" : "Wind", value: `${selected.wind.toFixed(0)} km/h`, sub: `${compass(selected.windDirection, rtl)} · ${selected.gust.toFixed(0)} ${rtl ? "هبات" : "gust"}`, icon: Wind, emphasis:"wind" };
  ...
  const activityMetrics: Metric[] = activity === "fishing" ? [
    windMetric,
    waveMetric,
      ...
      <section id="conditions" className="dashboard-wrap">
        ...
        <div className={`condition-board ${loading ? "is-loading" : ""}`}>
          ...
          <div className="metrics-grid">
            {activityMetrics.map((metric) => { const Icon = metric.icon; return <article className={`metric-card ${metric.emphasis ? `metric-emphasis metric-${metric.emphasis}` : ""}`} key={metric.label}><div className="metric-icon"><Icon size={20} /></div><div><p>{metric.label}</p><strong>{metric.value}</strong><span>{metric.sub}</span></div></article>; })}
        ...
        <div className="forecast-explorer">
          ...
          {activity === "fishing" && <>
            <ForecastChart title={rtl ? "المد والجزر" : "Tide"} icon={Waves} unit="m" values={conditions.hourly.map((hour) => hour.tide)} selectedHour={selectedHour} onSelect={setSelectedHour} accent="blue" tideDetails={{ wind: selected.wind, windDirection: selected.windDirection, wave: selected.wave, current: selected.current, currentDirection: selected.currentDirection }} nowHour={liveNowHour} nowTime={currentTime} rtl={rtl}>
          ...
          {activity === "kayaking" && <>
            ...
            <div className="chart-grid">
              <ForecastChart title={rtl ? "ارتفاع موج الكاياك" : "Kayak Wave Height"} icon={Waves} unit="m" values={conditions.hourly.map((hour) => hour.wave)} selectedHour={selectedHour} onSelect={setSelectedHour} accent="cyan" nowHour={liveNowHour} nowTime={currentTime} rtl={rtl}><div className="chart-facts compact"><span>{rtl ? "فترة الموج" : "Wave period"}<strong>{selected.wavePeriod.toFixed(1)}s</strong></span><span>{rtl ? "الرؤية" : "Visibility"}<strong>{selected.visibility.toFixed(0)} km</strong></span></div></ForecastChart>
        ...
        <div className="forecast-explorer weekly-explorer">
          <div className="section-title forecast-title"><div><p>{rtl ? spot.ar : spot.en} · {rtl ? "توقعات 7 أيام" : "7-day forecast"}</p><h2>{rtl ? "خطط لأسبوعك" : "Plan your week"}</h2><span>{rtl ? "حرّك مؤشر اليوم — الرسوم الثلاثة تتحرك معك" : "Drag the day marker — all three charts move with you"}</span></div><div className="selected-time selected-day"><small>{rtl ? "اليوم المختار" : "Selected day"}</small><strong>{selectedWeekLabel}</strong></div></div>
          <div className="week-scrubber"><span>{weekLabels[0]?.primary}</span><input dir="ltr" aria-label={rtl ? "اختر يوماً من الأسبوع" : "Select a day of the week"} type="range" min="0" max="6" step="1" value={selectedDay} onChange={(event) => selectWeekDay(Number(event.target.value))} /><span>{weekLabels[6]?.primary}</span></div>
          {activity === "fishing" && <>
            <ForecastChart title={rtl ? "المد والجزر" : "Tide"} icon={Waves} unit="m" values={weeklyData.map((day) => day.representative.tide)} selectedHour={selectedDay} onSelect={selectWeekDay} accent="blue" tideDetails={{ wind: selectedWeek.representative.wind, windDirection: selectedWeek.representative.windDirection, wave: selectedWeek.representative.wave, current: selectedWeek.representative.current, currentDirection: selectedWeek.representative.currentDirection }} xLabels={weekLabels} selectedLabel={selectedWeekLabel} rangeLabel={rtl ? "اختر يوم المد والجزر" : "Select tide forecast day"} rtl={rtl}>
          ...
          {activity === "kayaking" && <>
            ...
            <div className="chart-grid">
              <ForecastChart title={rtl ? "ارتفاع موج الكاياك الأسبوعي" : "Weekly Kayak Wave Height"} icon={Waves} unit="m" values={weeklyData.map((day) => day.wave)} selectedHour={selectedDay} onSelect={selectWeekDay} accent="cyan" xLabels={weekLabels} selectedLabel={selectedWeekLabel} rangeLabel={rtl ? "اختر يوم الموج" : "Select wave forecast day"} rtl={rtl}><div className="chart-facts compact"><span>{rtl ? "متوسط الموج" : "Average wave"}<strong>{selectedWeek.wave.toFixed(1)} m</strong></span><span>{rtl ? "أفضل وقت" : "Best time"}<strong>{formatTime12(selectedWeek.representative.time)}</strong></span></div></ForecastChart>
