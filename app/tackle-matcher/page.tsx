...
export default function TackleMatcherPage(){
  ...
  return <main dir={rtl?"rtl":"ltr"} className={`matcher-page ${rtl?"font-arabic":""}`}>
    <header className="matcher-topbar"><a href="/" className="back-link"><ArrowLeft size={17}/>{rtl?"العودة لحالة البحر":"Back to conditions"}</a><a className="brand" href="/"><span className="brand-mark"><Waves size={22}/></span><span>ALEX<strong>FISHER</strong><small>TACKLE MATCHER</small></span></a><nav><a href="/learning">{rtl?"تعلم":"Learning"}</a><a href="/community">{rtl?"المجتمع":"Community"}</a><a href="/shop">{rtl?"المتجر":"Shop"}</a></nav><button className="language-button" onClick={toggleLanguage}><Languages size={16}/>{language==="en"?"العربية":"EN"}</button></header>
