...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, CircleDot, Fish, Gauge, Languages, Layers3, Ruler, Search, ShieldCheck, Sparkles, Waves } from "lucide-react";

type Language = "en" | "ar";
type Mode = "build" | "reel" | "rod";
type RegionId = "med" | "red" | "suez" | "aqaba";
type StyleId = "surf" | "casting" | "popping" | "shore-jig" | "boat-jig";
type EnvironmentId = "sand" | "open" | "rocks" | "reef" | "harbour" | "structure" | "deep";
type FishProfile = { id:string; en:string; ar:string; scientific:string; regions:RegionId[]; styles:StyleId[]; fight:number; toothed?:boolean; abrasion?:boolean; tackle:string };

const regions = [
  { id:"med" as const, en:"Mediterranean Sea", ar:"البحر المتوسط" },
  { id:"red" as const, en:"Red Sea", ar:"البحر الأحمر" },
  { id:"suez" as const, en:"Gulf of Suez", ar:"خليج السويس" },
  { id:"aqaba" as const, en:"Gulf of Aqaba", ar:"خليج العقبة" },
];
const styles = [
...
const reelSizes = [1000,2000,2500,3000,4000,5000,6000,8000,10000,14000,18000];

const fish: FishProfile[] = [
  {id:"bluefish",en:"Bluefish",ar:"مياس",scientific:"Pomatomus saltatrix",regions:["med","suez"],styles:["casting","shore-jig","surf"],fight:7,toothed:true,tackle:"Minnow · metal lure · pencil"},
  {id:"barracuda",en:"Barracuda",ar:"براكودا",scientific:"Sphyraena spp.",regions:["med","red","suez","aqaba"],styles:["casting","popping","shore-jig"],fight:7,toothed:true,tackle:"Minnow · stickbait · metal jig"},
  {id:"needlefish",en:"Needlefish / Garfish",ar:"خرمان / إبرة",scientific:"Belonidae",regions:["med","red","suez","aqaba"],styles:["casting","surf"],fight:3,tackle:"Small minnow · float bait"},
  {id:"seabass",en:"Sea Bass",ar:"قاروص",scientific:"Dicentrarchus labrax",regions:["med"],styles:["casting","surf"],fight:5,tackle:"Minnow · soft plastic"},
  {id:"seabream",en:"Sea Bream",ar:"دنيس",scientific:"Sparus aurata",regions:["med","suez"],styles:["surf","casting"],fight:4,abrasion:true,tackle:"Bait rig · small soft lure"},
  {id:"mormora",en:"Mormora",ar:"مرمار",scientific:"Lithognathus mormyrus",regions:["med"],styles:["surf"],fight:3,tackle:"Bottom bait rig"},
  {id:"bonito",en:"Bonito",ar:"بلاميطة",scientific:"Sarda sarda",regions:["med","red","suez"],styles:["casting","shore-jig","boat-jig"],fight:8,tackle:"Metal jig · fast minnow"},
  {id:"little-tunny",en:"Little Tunny",ar:"تونة صغيرة",scientific:"Euthynnus alletteratus",regions:["med","red"],styles:["casting","shore-jig","boat-jig"],fight:9,tackle:"Metal jig · stickbait"},
  {id:"mackerel",en:"Mackerel",ar:"ماكريل",scientific:"Scomber spp.",regions:["med","red"],styles:["casting","shore-jig","boat-jig"],fight:4,tackle:"Small jig · sabiki"},
  {id:"leerfish",en:"Leerfish",ar:"ليتشة",scientific:"Lichia amia",regions:["med"],styles:["casting","popping","shore-jig"],fight:8,tackle:"Pencil · popper · minnow"},
  {id:"meagre",en:"Meagre / Corvina",ar:"لوت",scientific:"Argyrosomus regius",regions:["med","suez"],styles:["surf","casting","shore-jig"],fight:7,tackle:"Large soft plastic · bait"},
  {id:"grouper",en:"Grouper",ar:"وقار",scientific:"Epinephelinae",regions:["med","red","suez","aqaba"],styles:["shore-jig","boat-jig"],fight:8,abrasion:true,tackle:"Heavy jig · live bait"},
  {id:"amberjack",en:"Amberjack",ar:"أمبرجاك",scientific:"Seriola dumerili",regions:["med","red","suez","aqaba"],styles:["popping","shore-jig","boat-jig"],fight:10,abrasion:true,tackle:"Speed jig · stickbait"},
  {id:"mahi",en:"Mahi Mahi / Dorado",ar:"ماهي ماهي",scientific:"Coryphaena hippurus",regions:["med","red","suez"],styles:["casting","popping","boat-jig"],fight:8,tackle:"Pencil · popper · jig"},
  {id:"squid",en:"Squid / Calamari",ar:"سبيط / كاليماري",scientific:"Teuthida",regions:["med","red","suez","aqaba"],styles:["casting"],fight:2,tackle:"Egi #2.5–#3.5"},
  {id:"queenfish",en:"Queenfish",ar:"كوين فيش",scientific:"Scomberoides spp.",regions:["red","suez","aqaba"],styles:["casting","popping","shore-jig"],fight:8,tackle:"Pencil · popper · metal jig"},
  {id:"gt",en:"Giant Trevally / GT",ar:"جايانت تريفالي",scientific:"Caranx ignobilis",regions:["red","aqaba"],styles:["popping","shore-jig","boat-jig"],fight:10,abrasion:true,tackle:"Heavy popper · stickbait"},
  {id:"snapper",en:"Snapper",ar:"نهاش",scientific:"Lutjanidae",regions:["red","suez","aqaba"],styles:["shore-jig","boat-jig"],fight:7,abrasion:true,tackle:"Jig · bait"},
  {id:"kingfish",en:"Spanish Mackerel / Kingfish",ar:"دراك / كنعد",scientific:"Scomberomorus commerson",regions:["red","suez","aqaba"],styles:["casting","popping","shore-jig","boat-jig"],fight:9,toothed:true,tackle:"Long minnow · jig · stickbait"},
  {id:"dogtooth",en:"Dogtooth Tuna",ar:"تونة دوج توث",scientific:"Gymnosarda unicolor",regions:["red","aqaba"],styles:["popping","boat-jig"],fight:10,toothed:true,abrasion:true,tackle:"Heavy jig · popper"},
  {id:"yellowfin",en:"Yellowfin Tuna",ar:"تونة صفراء الزعنفة",scientific:"Thunnus albacares",regions:["red"],styles:["popping","boat-jig"],fight:10,tackle:"Heavy popper · jig"},
  {id:"cobia",en:"Cobia",ar:"كوبيـا",scientific:"Rachycentron canadum",regions:["red","suez"],styles:["popping","shore-jig","boat-jig"],fight:9,tackle:"Jig · large soft plastic"},
  {id:"emperor",en:"Emperor",ar:"شعري",scientific:"Lethrinidae",regions:["red","suez","aqaba"],styles:["shore-jig","boat-jig"],fight:6,abrasion:true,tackle:"Jig · bait"},
];
...
export default function TackleMatcherPage(){
  ...
  useEffect(()=>{const saved=window.localStorage.getItem("alexfisher-language");if(saved==="ar"||saved==="en")setLanguage(saved);const params=new URLSearchParams(window.location.search);const nextRegion=params.get("region");const nextStyle=params.get("style");const nextFish=params.get("fish");const nextSize=params.get("size");if(regions.some(item=>item.id===nextRegion))setRegion(nextRegion as RegionId);if(styles.some(item=>item.id===nextStyle))setStyle(nextStyle as StyleId);if(nextFish)setFishId(nextFish);if(nextSize){const index=sizeBands.indexOf(nextSize);if(index>=0)setSizeIndex(index)}},[]);
  const toggleLanguage=()=>{const next=language==="en"?"ar":"en";setLanguage(next);window.localStorage.setItem("alexfisher-language",next)};
  const availableFish=useMemo(()=>fish.filter((item)=>item.regions.includes(region)&&item.styles.includes(style)&&(`${item.en} ${item.ar}`).toLowerCase().includes(fishSearch.toLowerCase())),[region,style,fishSearch]);
  useEffect(()=>{const valid=fish.find((item)=>item.id===fishId&&item.regions.includes(region)&&item.styles.includes(style));if(!valid){const first=fish.find((item)=>item.regions.includes(region)&&item.styles.includes(style));if(first)setFishId(first.id)}},[region,style,fishId]);
  const target=fish.find((item)=>item.id===fishId)??fish[0];
  const recommendation=useMemo(()=>{
    ...
    return {required,recommendedReel,rod,weights,braid,leader,score:Math.max(55,score),notes};
  },[sizeIndex,environment,style,target,mode,ownedReel,rodPeMin,rodPeMax,depth,current]);
  const steps=rtl?["ابدأ","المنطقة","الطريقة","السمكة","الحجم","البيئة","النتيجة"]:["Start","Region","Style","Fish","Size","Environment","Result"];
  ...
  return <main dir={rtl?"rtl":"ltr"} className={`matcher-page ${rtl?"font-arabic":""}`}>
    ...
    <section className="matcher-shell">
      ...
      <div className="matcher-stage">
        ...
        {step===1&&<><div className="stage-heading"><p>02</p><h2>{rtl?"أين ستصطاد؟":"Where will you fish?"}</h2><span>{rtl?"المنطقة تصفي أنواع الأسماك المتاحة في مصر.":"Your region filters the target species available in Egypt."}</span></div><div className="choice-grid region-grid">{regions.map((item)=><button key={item.id} className={region===item.id?"selected":""} onClick={()=>setRegion(item.id)}><Waves size={23}/><strong>{rtl?item.ar:item.en}</strong><small>Egypt</small></button>)}</div></>}
        ...
        {step===6&&<div className="matcher-result"><div className="result-head"><div><p>{rtl?"إعداد الصيد الخاص بك":"YOUR FISHING SETUP"}</p><h2>{rtl?target.ar:target.en}</h2><span>{rtl?styles[styleIndex(style)].ar:styles[styleIndex(style)].en} · {sizeBands[sizeIndex]} · {rtl?regions.find(item=>item.id===region)?.ar:regions.find(item=>item.id===region)?.en}</span></div><div className={recommendation.score>=90?"excellent":"good"}><strong>{recommendation.score}%</strong><span>{status}</span></div></div><p className="result-plain-summary">{rtl?`باختصار: قصبة ${mode==="rod"?`${rodLength.toFixed(1)} م`:recommendation.rod} مع ماكينة ${mode==="reel"?ownedReel:recommendation.recommendedReel} وخيط مجدول PE ${recommendation.required} وليدر ${recommendation.leader} رطل — عدة متوازنة لصيد ${target.ar} بطريقة ${styles[styleIndex(style)].ar}.`:`In short: a ${mode==="rod"?`${rodLength.toFixed(1)} m`:recommendation.rod} rod with a ${mode==="reel"?ownedReel:recommendation.recommendedReel} reel, PE ${recommendation.required} braid and a ${recommendation.leader} lb leader — a balanced setup for ${target.en} using ${styles[styleIndex(style)].en}.`}</p><div className="result-equipment-grid">
          ...
          <article><span><CircleDot size={21}/></span><p>{rtl?"الماكينة":"Reel"}</p><h3>{mode==="reel"?ownedReel:recommendation.recommendedReel}</h3><dl><div><dt>{rtl?"السحب":"Drag"}</dt><dd>{Math.max(4,Math.round(recommendation.braid*.22))}–{Math.max(6,Math.round(recommendation.braid*.32))} kg</dd></div><div><dt>{rtl?"السعة":"Capacity"}</dt><dd>PE {recommendation.required} · 200–300 m</dd></div></dl></article>
          ...
          <article><span><ShieldCheck size={21}/></span><p>{rtl?"الليدر":"Leader"}</p><h3>{recommendation.leader} lb</h3><dl><div><dt>{rtl?"الخامة":"Material"}</dt><dd>{target.toothed?"Heavy fluorocarbon / wire option":"Fluorocarbon"}</dd></div><div><dt>{rtl?"الطول":"Length"}</dt><dd>{style==="surf"?"1.5–3 m":"0.8–1.5 m"}</dd></div></dl></article>
