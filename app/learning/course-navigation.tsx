...
import {lessonProgress} from "@/lib/learning-progress";
...
export function CourseProgress({courses,completed,rtl,pending}:{courses:Course[];selected:string;completed:string[];rtl:boolean;pending:boolean}){
  const lessons=courses.flatMap(course=>course.sections.flatMap(section=>section.lessons)),{done,total,percent}=lessonProgress(lessons,completed);
  return <Popover.Root><Popover.Trigger className="classroom-progress-trigger" aria-label={`${rtl?"تقدمك":"Your progress"}: ${percent}%`}><span className="classroom-progress-ring"><svg viewBox="0 0 44 44" aria-hidden="true"><circle cx="22" cy="22" r="19"/><circle className="ring-value" cx="22" cy="22" r="19" pathLength="100" strokeDasharray={`${percent} 100`}/></svg><Trophy/></span><span>{rtl?"تقدمك":"Your progress"}<small>{percent}%</small></span><ChevronDown/></Popover.Trigger><Popover.Portal><Popover.Content className="classroom-progress-popover" dir={rtl?"rtl":"ltr"} sideOffset={10} align="end" collisionPadding={12}><h2>{rtl?"تقدمك":"Your progress"}</h2><b>{rtl?"كل دروس الأكاديمية":"All academy lessons"}</b><p>{done} / {total} {rtl?"دروس مكتملة":"lessons completed"} · {percent}%</p><progress value={done} max={Math.max(1,total)} aria-label={rtl?"إكمال الكورس":"Overall completion"}/><small>{pending?(rtl?"جارٍ الحفظ…":"Saving…"):percent===100?(rtl?"أحسنت! أكملت جميع الدروس.":"Well done! You completed all lessons."):(rtl?"ضع علامة مكتمل بعد كل درس لتسجيل تقدمك.":"Mark each lesson complete to track your progress.")}</small><h3>{rtl?"كل الكورسات":"All courses"}</h3><ul>{courses.map(item=>{const all=item.sections.flatMap(section=>section.lessons),stats=lessonProgress(all,completed);return <li key={item.id}><span>{rtl?item.ar:item.en}</span><b>{stats.done}/{stats.total} · {stats.percent}%</b></li>})}</ul><Popover.Close className="classroom-progress-close" aria-label={rtl?"إغلاق التقدم":"Close progress"}><X/></Popover.Close></Popover.Content></Popover.Portal></Popover.Root>;
}
